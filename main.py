...
from third_party.remote_controller__widget_with_webserver import (
    frameless_widget_with_webserver,
)
from third_party.show_target_icon__behind_cursor import (
    main as show_target_icon__behind_cursor,
)

import third_party

sys.path.insert(0, third_party.__path__[0])
import full_black_screen_close_manual_with_animations
...
URL_REMOTE_SCREENSHOT = f"http://127.0.0.1:%s/command/{PORT_REMOTE_SCREENSHOT}"
...
def show_cursor_as_target():
    script_file_name = show_target_icon__behind_cursor.__file__
    subprocess.Popen([sys.executable, script_file_name, "1000"])


def full_black_screen():
    script_file_name = full_black_screen_close_manual_with_animations.__file__
